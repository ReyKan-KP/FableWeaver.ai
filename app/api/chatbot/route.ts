...
// Define the custom Message interface
interface Message {
  id: string;
  content: string;
  role: 'user' | 'assistant';
...
function createSystemPrompt(messages: Message[]): string {
  return ` Weaver Whisperer Chatbot Instructions
...
export async function POST(request: Request) {
  try {
    ...
    const user_input = latestMessage.content
    
    // Generate a UUID for interaction tracking
    const interactionId = crypto.randomUUID()
    
    // Log the interaction in Supabase if user is logged in
    if (session?.user?.id) {
      await supabase
    ...
    // Create the conversation history and system prompt
    ...
    // Create response in the expected format for our frontend
